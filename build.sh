...
doInit () {
  ...
  docker-compose run --rm -u "$USER_UID:$GROUP_GID" node pnpm install
}
...
build () {
  ...
  #build css
  docker-compose run -e SKIN_DIR=$SKIN_DIR -e SCSS_DIR=$SCSS_DIR -e DIST_DIR=$OVERRIDE_DIST --rm -u "$USER_UID:$GROUP_GID" node pnpm run release:prepare
  status=$?
  if [ $status != 0 ];
  then
    exit $status
  ...
  for dir in "${dirs[@]}"; do
    ...
    docker-compose run -e SKIN_DIR=$SKIN_DIR -e SCSS_DIR=$SCSS_DIR -e DIST_DIR=$OVERRIDE_DIST -e SKIN=$tmp  --rm -u "$USER_UID:$GROUP_GID" node pnpm run sass:build:release
    status=$?
    if [ $status != 0 ];
    then
      exit $status
  ...
  docker-compose run -e OVERRIDE_SRC=$OVERRIDE_SRC -e OVERRIDE_DIST=$OVERRIDE_DIST --rm -u "$USER_UID:$GROUP_GID" node pnpm run override:i18n
  status=$?
  if [ $status != 0 ];
  then
    exit $status
...
watch () {
  docker-compose run --rm -u "$USER_UID:$GROUP_GID" node pnpm run dev:watch
}

lint () {
  docker-compose run --rm -u "$USER_UID:$GROUP_GID" node pnpm run dev:lint
}

lint-fix () {
  docker-compose run --rm -u "$USER_UID:$GROUP_GID" node pnpm run dev:lint-fix
}
...
publishNPM () {
  ...
  docker-compose run --rm -u "$USER_UID:$GROUP_GID" node pnpm publish --no-git-checks --tag $LOCAL_BRANCH
  status=$?
  if [ $status != 0 ];
  then
    exit $status
